...
export function Reportes() {
  ...
  // Juntar gastos de cajas cerradas + caja actual abierta
  const todasCajas = cajaAbierta
    ? [...historialCajas, { ...cajaAbierta, movimientos: movimientosCaja }]
    : historialCajas;

  const gastosDelPeriodo = todasCajas.flatMap(caja =>
    (caja.movimientos || []).filter(m => {
      if (m.tipo !== 'Gasto / Salida') return false;
  ...
  const imprimirPDF = () => {
    ...
    const filasVentas = filtradas.length > 0
      ? filtradas.map(v => `
          <tr>
          ...
          <tr class="total-row">
            <td colspan="6">TOTAL VENTAS</td>
            <td style="text-align:right">S/ ${totalVentas.toFixed(2)}</td>
          </tr>`
      : '<tr><td colspan="7" style="text-align:center;color:#999;padding:16px">Sin ventas en el período</td></tr>';

    const filasGastos = gastosDelPeriodo.length > 0
      ? gastosDelPeriodo.map(m => `
          <tr>
            <td>${new Date(m.fecha).toLocaleString('es-PE', { dateStyle:'short', timeStyle:'short' })}</td>
            <td>${m.concepto}</td>
            <td>${m.usuario || '—'}</td>
            <td style="text-align:right;font-weight:600;color:#c0392b">S/ ${parseFloat(m.monto).toFixed(2)}</td>
          </tr>`).join('') + `
          <tr class="total-row-red">
            <td colspan="3">TOTAL GASTOS</td>
            <td style="text-align:right;color:#c0392b">S/ ${totalGastos.toFixed(2)}</td>
          </tr>`
    ...
    win.document.write(`<!DOCTYPE html><html lang="es"><head>
      ...
      <style>
        * { margin:0; padding:0; box-sizing:border-box; }
        ...
        .header { display:flex; justify-content:space-between; align-items:flex-start; border-bottom: 3px solid #2d5a27; padding-bottom: 16px; margin-bottom: 20px; }
        .brand h1 { font-size: 22px; color: #2d5a27; font-weight: 700; }
        .brand p { color: #666; font-size: 11px; }
        .reporte-info { text-align: right; }
        .reporte-info h2 { font-size: 15px; font-weight: 700; color: #2d5a27; }
        .reporte-info p { color: #666; font-size: 11px; margin-top: 3px; }
        .resumen { display: grid; grid-template-columns: repeat(5, 1fr); gap: 10px; margin-bottom: 16px; }
        .stat { background: #f4f7f3; border: 1px solid #e2e8df; border-radius: 8px; padding: 10px 14px; }
        .stat-label { font-size: 9px; font-weight: 700; color: #718096; text-transform: uppercase; letter-spacing: 0.5px; margin-bottom: 4px; }
        .stat-value { font-size: 17px; font-weight: 700; }
        .stat-value.green { color: #2d5a27; }
        .stat-value.red { color: #c0392b; }
        .neto-box { background: #2d5a27; color: white; border-radius: 8px; padding: 12px 20px; display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px; }
        .neto-box .label { font-size: 12px; opacity: 0.85; }
        .neto-box .value { font-size: 22px; font-weight: 700; }
        h3 { font-size: 12px; font-weight: 700; margin: 16px 0 8px; color: #2d5a27; border-left: 3px solid #2d5a27; padding-left: 8px; }
        table { width: 100%; border-collapse: collapse; margin-bottom: 8px; font-size: 11px; }
        thead { background: #2d5a27; }
        th { padding: 6px 10px; text-align: left; color: white; font-weight: 600; font-size: 10px; text-transform: uppercase; }
        td { padding: 6px 10px; border-bottom: 1px solid #e2e8df; }
        tr:nth-child(even) td { background: #f9fafb; }
        .total-row td { font-weight: 700; background: #e8f0e6 !important; }
        .total-row-red td { font-weight: 700; background: #fdecea !important; }
        ...
        .footer { margin-top: 24px; border-top: 1px solid #e2e8df; padding-top: 10px; display: flex; justify-content: space-between; color: #999; font-size: 10px; }
      ...
      <table>
        <thead><tr><th>Código</th><th>Fecha</th><th>Cliente</th><th>Vendedor</th><th>Método pago</th><th>Tipo</th><th style="text-align:right">Total</th></tr></thead>
        <tbody>${filasVentas}</tbody>
      </table>

      <h3>Gastos / Salidas de caja</h3>
      <table>
        <thead><tr><th>Fecha</th><th>Concepto</th><th>Usuario</th><th style="text-align:right">Monto</th></tr></thead>
      ...
      <table>
        <thead><tr><th>#</th><th>Producto</th><th style="text-align:right">Cantidad</th><th style="text-align:right">Total</th></tr></thead>
        <tbody>
  ...
  return (
    <div>
      <div className="page-header"><h1>Reportes</h1><p>Exporta y genera reportes de ventas y gastos</p></div>
      <div className="card" style={{maxWidth:600}}>
        <h3 style={{marginBottom:20,fontSize:16}}>Reporte de ventas y gastos</h3>
    ...
    <div>
      ...
      {modal && (
        <div className="modal-overlay">
          <div className="modal">
            <div className="modal-header"><h2>Nuevo usuario</h2>
