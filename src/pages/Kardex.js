...
export default function Kardex() {
  ...
  const filtered = kardex.filter((k) => {
    const fechaMovimiento = new Date(k.fecha).toISOString().split("T")[0];

    return (
      (!filtroProducto || k.productoId === Number(filtroProducto)) &&
      (!filtroTipo || k.tipo === filtroTipo) &&
      (!filtroFecha || fechaMovimiento === filtroFecha)
    );
  });
            ...
            {filtered.length === 0 ? (
              <tr>
                <td
                  ...
                  style={{ padding: 40, color: "var(--text-light)" }}
                >
                  Sin movimientos
                </td>
              </tr>
              ...
              filtered.map((k) => (
                <tr key={k.id}>
                  ...
                  <td>
                    <strong>{k.producto}</strong>
                  </td>
                  <td>
                    <span
                      className={`badge ${k.tipo === "Venta" ? "badge-red" : k.tipo === "Ingreso" ? "badge-green" : "badge-orange"}`}
                    >
                      {k.tipo}
                    </span>
                  </td>
                  <td
                    ...
                    style={{
                      fontWeight: 600,
                      color: k.deltaSacos >= 0 ? "var(--green)" : "var(--red)",
                    }}
                  >
                    {k.deltaSacos > 0 ? "+" : ""}
                    {k.deltaSacos}
                  ...
                  <td
                    ...
                    style={{
                      fontWeight: 600,
                      color: k.deltaKg >= 0 ? "var(--green)" : "var(--red)",
                    }}
                  >
                    {k.deltaKg > 0 ? "+" : ""}
                    {k.deltaKg?.toFixed(2)}
